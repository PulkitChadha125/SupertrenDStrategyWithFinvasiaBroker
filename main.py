...
def process_data(data_dict):
    ...
    for strategy_tag, data in data_dict.items():
        if pd.notna(strategy_tag):
            ...
            # for strategy_tag, data in data_dict.items():
            #     if pd.notna(strategy_tag):
            df = pd.read_csv(f'{strategy_tag}.csv')
            ...
            if start_time and end_time and start_time <= current_time  <= end_time:
                ...
                if last_supertrend_signal == -1 and signal_dict[strategy_tag]['Sell'] == False and signal_dict[strategy_tag]['IntialTrade'] == False and int(signal_dict[strategy_tag]['Count']) <= int(signal_dict[strategy_tag]['max_trades']):
                ...
                df.to_csv(f'{strategy_tag}.csv')


def write_to_order_logs(message):
    with open('order_logs.txt', 'a') as file:
        file.write(message + '\n')
...
def tp_and_sl(signal_dict):
    ...
    for strategy_tag, data in data_dict.items():
        ...
        if pd.notna(strategy_tag):
            ...
            if current_time == end_time_str and signal_dict[strategy_tag]['S'] == True and signal_dict[strategy_tag]['T'] == True and signal_dict[strategy_tag]['Buy'] == True and int(finvasia.get_position_detail(symbol_to_find=signal_dict[strategy_tag]['PreviousString'])) > 0:
                signal_dict[strategy_tag]['T'] = False
                signal_dict[strategy_tag]['S'] = False
                orderlog = f"{timestamp} Open Position Exit by strategy stop time for  {strategy_tag}  For option contract  = {signal_dict[strategy_tag]['PreviousString']} @ {option_script_ltp}"
                finvasia.sell_order(tradingsymbol=signal_dict[strategy_tag]['PreviousString'],
            ...
            if current_time == end_time_str and signal_dict[strategy_tag]['S'] == True and signal_dict[strategy_tag][ 'T'] == True and signal_dict[strategy_tag]['Sell'] == True and int(finvasia.get_position_detail(symbol_to_find=signal_dict[strategy_tag]['PreviousString'])) > 0:
                signal_dict[strategy_tag]['T'] = False
                signal_dict[strategy_tag]['S'] = False
                orderlog = f"{timestamp} Open Position Exit by strategy stop time for  {strategy_tag}  For option contract  = {signal_dict[strategy_tag]['PreviousString']} @ {option_script_ltp}"
                finvasia.sell_order(tradingsymbol=signal_dict[strategy_tag]['PreviousString'],
