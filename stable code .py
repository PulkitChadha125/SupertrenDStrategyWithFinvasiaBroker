...
def tp_and_sl(signal_dict):
    ...
    for strategy_tag, data in data_dict.items():
        if pd.notna(strategy_tag):
            ...
            if current_time >= end_time_str and signal_dict[strategy_tag]['S'] == True and signal_dict[strategy_tag][
                'T'] == True and signal_dict[strategy_tag]['Buy'] == True and int(
            ...
            if current_time >= end_time_str and signal_dict[strategy_tag]['S'] == True and signal_dict[strategy_tag][
                'T'] == True and signal_dict[strategy_tag]['Sell'] == True and int(
                finvasia.get_position_detail(symbol_to_find=signal_dict[strategy_tag]['PreviousString'])) > 0:
                signal_dict[strategy_tag]['T'] = False
                signal_dict[strategy_tag]['S'] = False
                orderlog = f"{timestamp} Open Position Exit by strategy stop time for  {strategy_tag}  For option contract  = {signal_dict[strategy_tag]['PreviousString']} @ {option_script_ltp}"
